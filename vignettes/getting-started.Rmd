...
- `//OP-PLUS | //OP-MINUS | //OP-TIMES | //OP-DIVIDE` - Arithmetic operators
...
Before writing a linter, see whether lintr already has one. The rules above are
useful as teaching examples, but in real projects use lintr's own:
...
lintr::lint("my_script.R", linters = lintr::undesirable_function_linter())

# The full list
lintr::available_linters()
```

`undesirable_function_linter()` covers discouraged calls such as `attach()` and
`sapply()` — see `lintr::default_undesirable_functions` for the list it uses.
...
- Revisit [Common XPath Patterns](#common-xpath-patterns) and the [Creating Linters Without XPath](no-xpath-guide.html) vignette for reference
