...
`lintrhelper` makes it easy to create custom linters for the `lintr` package. This vignette will walk you through the basics of creating and testing custom linters.
...
Let's create a simple linter that warns against using `T` and `F` instead of `TRUE` and `FALSE`:
...
- **xpath**: An XPath expression that matches the problematic code patterns in R's parse tree
- **message**: The message to show users when this pattern is found
- **linter_name**: A unique identifier for your linter
- **type**: Severity level - "style", "warning", or "error"
